...
# EfficientNet Baseline model information:
base_model = [
...
# depth = alpha ^ phi, width = beta ^ phi etc.
phi_values = {
    # tuple values: (phi_value, resolution, drop_rate)
    'b0': (0, 224, 0.2),
    'b1': (0.5, 240, 0.2),
    'b2': (1, 260, 0.3),
    'b3': (2, 300, 0.3),
...
class CNNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, groups=1):
        ...
        # if groups=1 --> Normal Convolution, if groups=in_channels --> Depthwise Convolution
...
# Squeeze and Excitation to get attention scores for each channel
...
# the Inverted Residual Block / MBConv(as mentioned in the paper)
class MBConv(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.use_residual:
            return self.stocahstic_depth(self.conv(x)) + inputs
        else:
            return self.conv(x)
    ...
    def create_features(self, width_factor, depth_factor, last_channels):
        ...
        for expand_ratio, channels, repeats, stride, kernel_size in base_model:
            ...
            for layer in range(layers_repeats):
                features.append(
                    MBConv(in_channels, out_channels, expand_ratio=expand_ratio,
                           stride=stride if layer == 0 else 1, kernel_size=kernel_size, padding=kernel_size//2)
                )
